
import Form from "next/form";
import { serverAction1 } from "./serverAction1";

export default function Client1() {
  const dummyHrs = [];
  for (let i = 1; i <= 12; i++) {
    if (i < 10) dummyHrs.push("0" + i);
    else dummyHrs.push(String(i));
  }

  const dummyMinutes = [];
  for (let i = 0; i <= 59; i++) {
    if (i < 10) dummyMinutes.push("0" + i);
    else dummyMinutes.push(String(i));
  }

  const meridiem = ["AM", "PM"];

  return (
    <Form action={serverAction1} className="space-y-6 p-6 bg-gray-50 dark:bg-gray-900 rounded-md">
      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">
          Select Clinic Start time
        </label>
        <div className="flex gap-2 items-center">
          <select
            name="startHr"
            className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
          >
            {dummyHrs.map((hr) => (
              <option value={hr} key={hr}>
                {hr}
              </option>
            ))}
          </select>
          <select
            name="startMin"
            className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
          >
            {dummyMinutes.map((min) => (
              <option value={min} key={min}>
                {min}
              </option>
            ))}
          </select>
          <select
            name="startMeridiem"
            className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
          >
            {meridiem.map((mer) => (
              <option value={mer} key={mer}>
                {mer}
              </option>
            ))}
          </select>
        </div>
      </div>

      <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">
          Select Clinic End time
        </label>
        <div className="flex gap-2 items-center">
          <select
            name="endHr"
            className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
          >
            {dummyHrs.map((hr) => (
              <option value={hr} key={hr}>
                {hr}
              </option>
            ))}
          </select>
          <select
            name="endMin"
            className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
          >
            {dummyMinutes.map((min) => (
              <option value={min} key={min}>
                {min}
              </option>
            ))}
          </select>
          <select
            name="endMeridiem"
            className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
          >
            {meridiem.map((mer) => (
              <option value={mer} key={mer}>
                {mer}
              </option>
            ))}
          </select>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">Break Start</label>
          <div className="flex gap-2 items-center">
            <select
              name="breakStartHr"
              className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
            >
              {dummyHrs.map((hr) => (
                <option value={hr} key={hr}>
                  {hr}
                </option>
              ))}
            </select>
            <select
              name="breakStartMin"
              className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
            >
              {dummyMinutes.map((min) => (
                <option value={min} key={min}>
                  {min}
                </option>
              ))}
            </select>
            <select
              name="breakStartMeridiem"
              className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
            >
              {meridiem.map((mer) => (
                <option value={mer} key={mer}>
                  {mer}
                </option>
              ))}
            </select>
          </div>
        </div>

        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">Break End</label>
          <div className="flex gap-2 items-center">
            <select
              name="breakEndHr"
              className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
            >
              {dummyHrs.map((hr) => (
                <option value={hr} key={hr}>
                  {hr}
                </option>
              ))}
            </select>
            <select
              name="breakEndMin"
              className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
            >
              {dummyMinutes.map((min) => (
                <option value={min} key={min}>
                  {min}
                </option>
              ))}
            </select>
            <select
              name="breakEndMeridiem"
              className="border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100"
            >
              {meridiem.map((mer) => (
                <option value={mer} key={mer}>
                  {mer}
                </option>
              ))}
            </select>
          </div>
        </div>
      </div>

      <div>
        <button
          type="submit"
          className="mt-2 inline-flex items-center justify-center rounded-md bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 px-4 py-2 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
        >
          Submit
        </button>
      </div>
    </Form>
  );
}



// import Form from "next/form";
// import { serverAction1 } from "./serverAction1";

// export default function Client1() {
//     const dummyHrs = [];
//     for (let i = 1; i <= 12; i++) {
//         if (i < 10) dummyHrs.push("0" + i);
//         else dummyHrs.push(String(i));
//     }

//     const dummyMinutes = [];
//     for (let i = 0; i <= 59; i++) {
//         if (i < 10) dummyMinutes.push("0" + i);
//         else dummyMinutes.push(String(i));
//     }

//     const meridiem = ["AM", "PM"];

//     return (
//         <Form action={serverAction1}>
//             <label>Select Clinic Start time</label>
//             <select name="startHr">
//                 {dummyHrs.map((hr) => <option value={hr} key={hr}>{hr}</option>)}
//             </select>
//             <select name="startMin">
//                 {dummyMinutes.map((min) => <option value={min} key={min}>{min}</option>)}
//             </select>
//             <select name="startMeridiem">
//                 {meridiem.map((mer) => <option value={mer} key={mer}>{mer}</option>)}
//             </select>

//             <label>Select Clinic End time</label>
//             <select name="endHr">
//                 {dummyHrs.map((hr) => <option value={hr} key={hr}>{hr}</option>)}
//             </select>
//             <select name="endMin">
//                 {dummyMinutes.map((min) => <option value={min} key={min}>{min}</option>)}
//             </select>
//             <select name="endMeridiem">
//                 {meridiem.map((mer) => <option value={mer} key={mer}>{mer}</option>)}
//             </select>

//             <label>breakStartHr</label>
//             <select name="breakStartHr">
//                 {dummyHrs.map((hr) => <option value={hr} key={hr}>{hr}</option>)}
//             </select>
//             <label>breakStartMin</label>
//             <select name="breakStartMin">
//                 {dummyMinutes.map((min) => <option value={min} key={min}>{min}</option>)}
//             </select>
//             <select name="breakStartMeridiem">
//                 {meridiem.map((mer) => <option value={mer} key={mer}>{mer}</option>)}
//             </select>

//             <label>breakEndHr</label>
//             <select name="breakEndHr">
//                 {dummyHrs.map((hr) => <option value={hr} key={hr}>{hr}</option>)}
//             </select>
//             <label>breakEndMin</label>
//             <select name="breakEndMin">
//                 {dummyMinutes.map((min) => <option value={min} key={min}>{min}</option>)}
//             </select>
//             <select name="breakEndMeridiem">
//                 {meridiem.map((mer) => <option value={mer} key={mer}>{mer}</option>)}
//             </select>

//             <button type="submit">Submit</button>
//         </Form>
//     );
// }