...
export default async function SessionManagment() {
    ...
    const fetchRecord = await db.execute(`SELECT day, start_time AS start, end_time AS end, break_start AS breakStart, break_end AS breakEnd, buffer_minutes AS buffer FROM slotPerDay;`);
    ...
    return (<>
        ...
        {daysWithoutSlots.map((day) => (
            <div key={day} className="border-2 m-4">
                <span>{day} </span>
                <Link href={`/manage-slots/day?d=${day}`}>Generate</Link>
            </div>
        ))}
        <div>
            <h2>Current Slots </h2>
            {currentSlots.map((daySlot) => (
                <div key={daySlot.day} className="border-2 m-4">
                    ...
                    <Link href={`/manage-slots/day?d=${daySlot.day}&edit=true`}>Edit</Link>
                </div>
            ))}
        </div>
