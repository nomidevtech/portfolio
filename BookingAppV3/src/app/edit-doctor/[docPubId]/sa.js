...
async function getTreatmentId(treatmentStr) {
    ...
    const [rawName, rawDuration] = parts;
    ...
    const result = await db.execute(
        `SELECT id FROM treatments 
         WHERE LOWER(name) = ? AND duration = ? AND admin_id = ?`,
...
export async function editDoctorServerAction(formData) {
    ...
    try {
        ...
        await db.execute(
            ...
            [name.toLowerCase(), department.toLowerCase(), doctorId, adminId]
        );
    } catch (error) {
        console.error(error);
        return null;
...
export async function removeDoctorTreatment(formData) {
    ...
    try {
        ...
        await db.execute(
            `DELETE FROM doctor_treatments 
             WHERE doctor_id = ? AND treatment_id = ? AND admin_id = ?`,
...
export async function addDoctorTreatment(formData) {
    ...
    try {
        ...
        const pubId = nanoid(12);

        await db.execute(
            `INSERT OR IGNORE INTO doctor_treatments 
             (public_id, admin_id, doctor_id, treatment_id) 
             VALUES (?, ?, ?, ?)`,
            [pubId, adminId, doctorId, treatmentId]
        );
    } catch (error) {
        console.error(error);
        return null;
