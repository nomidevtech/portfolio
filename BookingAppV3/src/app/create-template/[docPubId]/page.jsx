...
export default async function DoctorCreateTemplate({ params }) {
    ...
    const doctorName = name.split("-").map(fn => fn[0].toUpperCase() + fn.slice(1)).join(" ");
    ...
    return (
        <main className="page-shell">
            <div className="mb-8">
                ...
                <h1 className="mt-4 text-3xl font-black text-slate-950">Create template for Dr. {doctorName}</h1>
                <p className="mt-2 text-slate-600">
                    Department: {fetchDoctor.rows[0].department.split(" ").map(fn => fn[0].toUpperCase() + fn.slice(1)).join(" ")}
                </p>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
                <section className="section-panel">
                    ...
                    <div className="mt-5 grid gap-3">
                        {currentTemplates.length > 0 ? currentTemplates.map(temp => (
                            <div key={temp.public_id} className="data-card">
                                <div className="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <p className="text-lg font-bold text-slate-950">{getDayName(temp.day_number)}</p>
                                        <p className="mt-1 text-sm text-slate-600">Clinic: {minutesToMeridiem(temp.start_time, true)} - {minutesToMeridiem(temp.end_time, true)}</p>
                ...
                <Form action={createTemplateServerAction} className="form-panel grid gap-5">
                    ...
                    <TimeGroup title="Clinic start" prefix="start" hrs={dummyHrs} minutes={dummyMinutes} meridiem={meridiem} defaults={["09", "00", "AM"]} />
                    <TimeGroup title="Clinic end" prefix="end" hrs={dummyHrs} minutes={dummyMinutes} meridiem={meridiem} defaults={["05", "00", "PM"]} />
                    <TimeGroup title="Break start" prefix="breakStart" hrs={dummyHrs} minutes={dummyMinutes} meridiem={meridiem} defaults={["12", "00", "PM"]} />
                    <TimeGroup title="Break end" prefix="breakEnd" hrs={dummyHrs} minutes={dummyMinutes} meridiem={meridiem} defaults={["01", "00", "PM"]} />
                    <button type="submit" className="btn-primary w-full sm:w-auto">Create template</button>
                </Form>
            </div>
        </main>
    );
    ...
    return (
        <fieldset className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
            ...
            <div className="mt-3 grid grid-cols-3 gap-3">
                <select name={hourName} defaultValue={defaults[0]}>{hrs.map((hr) => <option value={hr} key={hr}>{hr}</option>)}</select>
                <select name={minuteName} defaultValue={defaults[1]}>{minutes.map((min) => <option value={min} key={min}>{min}</option>)}</select>
                <select name={meridiemName} defaultValue={defaults[2]}>{meridiem.map((mer) => <option value={mer} key={mer}>{mer}</option>)}</select>
            </div>
        </fieldset>
    );
}
