...
export default async function Authors({ searchParams }) {
  ...
  if (!value) return <div className="max-w-3xl mx-auto px-4 py-16"><p className="font-sans text-[var(--text-muted)]">No author specified.</p></div>;
  const author = value;
  const fetchPosts = await db.execute(`
    ...
    WHERE users.username = ? GROUP BY posts.id ORDER BY posts.created_at DESC
  `, [author]);
  if (!fetchPosts?.rows?.length) return <div className="max-w-3xl mx-auto px-4 py-16"><p className="font-sans text-[var(--text-muted)]">No posts by @{author}.</p></div>;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      <div className="mb-8 pb-4 border-b-2 border-[var(--text)]">
        <p className="font-sans text-xs uppercase tracking-widest text-[var(--accent)] mb-1">Author</p>
        <h1 className="text-2xl font-bold text-[var(--text)]">@{author}</h1>
