...
export default async function Taxonomies({ searchParams }) {
  ...
  if (!value) return <div className="max-w-3xl mx-auto px-4 py-16"><p className="font-sans text-[var(--text-muted)]">No category specified.</p></div>;
  ...
  const fetchPosts = await db.execute(`
    ...
    WHERE taxonomies.name = ? GROUP BY posts.id ORDER BY posts.created_at DESC
  `, [taxonomy]);
  if (!fetchPosts?.rows?.length) return <div className="max-w-3xl mx-auto px-4 py-16"><p className="font-sans text-[var(--text-muted)]">No posts in {taxonomy}.</p></div>;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      <div className="mb-8 pb-4 border-b-2 border-[var(--text)]">
        <p className="font-sans text-xs uppercase tracking-widest text-[var(--accent)] mb-1">Category</p>
        <h1 className="text-2xl font-bold text-[var(--text)]">{taxonomy}</h1>
